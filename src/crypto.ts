...
/**
 The following will not compile in browser bundles. When using rollup to build
 bundles the following code can be added to modify the following two lines
 for browser builds. The first import line is removed entirely, and the second
 relates subtle to window.crypto.subtle. There is likely to be a better way of
 achieving the result in the future and this approach should be revisited.
 
  replace({
    preventAssignment: true,
    'import * as crypto from \'crypto\';': '',
    'crypto.webcrypto': 'window.crypto.subtle'
  })
...
/**
 * Types of PEM key that can be used with the crypto class.
 */
...
export class Crypto {
  ...
  /**
   * Hash the provided array and return the hash as Uint8Array.
   * @param array 
   * @returns 
   */
  public static async hash(array: Uint8Array): Promise<Uint8Array> {
    return new Uint8Array(await subtle.digest('SHA-256', array));
  }
   ...
   */
  public static async sign(key: CryptoKey, data: Uint8Array)
    : Promise<Uint8Array> {
  ...
  /**
   * Turns the public or private key into a PEM format string. If the key is 
   * public then the SPKI format is used. If the key is private then the PKCS#8
   * format is used. Other types of key are not supported.
  ...
  public static async exportKey(key: CryptoKey): Promise<string> {
    ...
    switch(key.type) {
      ...
      case 'private':
        ...
        break;
      default:
        throw `${key.type} not supported`;
        break;
    }
    const k = new Uint8Array(await subtle.exportKey(format, key));
    let b = '';
  ...
  /**
   * Converts a PEM format public key into a crypto key that can be used to 
   * verify an OWID.
  ...
  /**
   * Converts a PEM format private key into a crypto key that can be used to 
   * sign an OWID.
